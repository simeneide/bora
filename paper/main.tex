...
\maketitle
\abstract{Lorem Ipsum; also remember to write about the learning rate $\tau$}
...
It is in principle possible to avoid the MAP maximisation, and instead take a fully Bayesian approach, which would need to rely on some form of MCMC.
However, MCMC methods
and in this sense might not scale. One advantage is the possibility to estimate $\tau$, and integrate over its empirical posterior distribution obtained within the MCMC. We have studied the effect of $tau$ in the test data. One can also cross-validate $tau$ in the training data. This would also be computationally too heavy. However, one could devise a way of subsampling the domain training data; for example, one could use only a few domains with large training data and few with little data, and run a rapid cross-validation for $tau$, the result of which would be used further. 

Another important discovery is the usefulness to let $tau$ determine the learning rate. The intiuition is clear and the effect potent. 
...
\begin{itemize}
    ...
    \item Prove that the optimal learning rate is indeed determined by $\tau$. 
\end{itemize}
